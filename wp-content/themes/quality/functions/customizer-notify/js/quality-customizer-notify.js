...
(function (api) {
	...
	api.sectionConstructor['quality-customizer-notify-section'] = api.Section.extend(
		{
				...
				return true;
					...
					jQuery( document ).ready(
						function () {
							...
							jQuery( '.quality-customizer-notify-dismiss-recommended-action' ).click(
								function () {
									...
									jQuery.ajax(
										{
											...
											dataType: 'html',
											url: qualityCustomizercompanionObject.ajaxurl,
											beforeSend: function () {
												jQuery( '#' + id ).parent().append( '<div id="temp_load" style="text-align:center"><img src="' + qualityCustomizercompanionObject.base_path + '/images/spinner-2x.gif" /></div>' );
											},
											success: function (data) {
												...
												var actions_count      = recommended_sction.find( '.quality-customizer-plugin-notify-actions-count' );
												var section_title      = recommended_sction.find( '.section-title' );
												jQuery( '.quality-customizer-plugin-notify-actions-count .current-index' ).text( index );
												container.slideToggle().remove();
												if (jQuery( '.quality-theme-recomended-actions_container > .epsilon-recommended-actions' ).length === 0) {

													actions_count.remove();

													if (jQuery( '.quality-theme-recomended-actions_container > .epsilon-recommended-plugins' ).length === 0) {
										...
										jQuery( '.quality-customizer-notify-dismiss-button-recommended-plugin' ).click(
											function () {
												var id = jQuery( this ).attr( 'id' ),
												action = jQuery( this ).attr( 'data-action' );
												jQuery.ajax(
												...
												if (activateButton.length) {
													...
													if (typeof url !== 'undefined') {
														...
														jQuery.ajax(
															{
																beforeSend: function () {
																	jQuery( activateButton ).replaceWith( '<a class="button updating-message">' + qualityCustomizercompanionObject.activating_string + '...</a>' );
																},
																async: true,
																type: 'GET',
																url: url,
